...
import { AsyncPipe, DatePipe, NgIf } from '@angular/common'; // Добавлен NgIf
...
@Component({
  ...
  standalone: true,
  imports: [AsyncPipe, MarkdownComponent, DatePipe, RouterLink, NgIf], // AsyncPipe и NgIf ОБЯЗАТЕЛЬНЫ здесь
  template: `
  <ng-container *ngIf="post$ | async as post">
  <article class="min-h-screen bg-[#0f011a] text-stone-200 selection:bg-cyan-500/30">

  <nav class="fixed top-12 left-12 z-50">
  <a routerLink="/blog" class="text-cyan-500 hover:text-cyan-300 transition-all font-mono text-xs uppercase tracking-[0.3em] bg-[#0f011a]/80 p-3 rounded-xl backdrop-blur-md border border-cyan-900/50 shadow-2xl no-underline">
  ← Назад в Блог
  </a>
  </nav>

  <header class="relative h-[55vh] flex items-end p-8 md:p-16 border-b border-violet-900 overflow-hidden pt-32">
  <ng-container *ngIf="post.attributes.coverImage">
  <img [src]="post.attributes.coverImage"
  class="absolute inset-0 w-full h-full object-cover opacity-30 scale-105 z-0"
  ...
  <h1 class="text-5xl md:text-7xl font-black italic text-white uppercase font-serif tracking-tighter leading-tight drop-shadow-2xl animate-pulse-slow text-left">
